...
const RoomDetails = () => {
    ...
    useEffect(() => {
  const roomData = roomsDummyData.find(room => room.id === id);
...
<div className='mb-6'>
         <img 
            ...
            alt="Back" 
            // 'rotate-180' turns the arrow left. Remove if your icon is already left-pointing.
            className='w-6 h-6 cursor-pointer hover:opacity-75 transition-opacity rotate-180' 
            onClick={() => navigate(-1)} // 4. Navigate back on click
         />
      </div>
   {/* room detailes */}
        ...
        <div className='grid grid-cols-2 gap-4 lg:w-1/2 w-full'>
            {room?.images.length > 1 && room.images.map((image, index)=>(
                <img onClick={()=>setMainImage(image)}
                 key={index} src={image} alt="Room Image" className={`w-full rounded-xl shadow-md object-cover cursor-pointer outline ${mainImage === image && 'outline-3 outline-orange-500'}`}/>
            ))}
        </div>
    </div>
      {/* Rooms Highlights */}
   ...
   <button type='submit' className='bg-primary hover:bg-primary-dull active:scale-95 transition-all text-white rounded-md max-md:w-full max-md:mt-6 md:px-25 py-3 md:py-4 text-base cursor-pointer'>
    ...
    <div>
      <p className='max-w-3xl border-y border-gray-300 my-15 py-10 text-gray-500'>
        Guest will be allocated on the ground floor according to availability. You get a comfortable two bedroom apartment has a true city feeling. The price quoted is for two guests, at the guest slot please mark the number of guests to get the exact price for group. The guest will be allocated ground floor according to availability. You get the comfortable to bedroom apartment that has a true city feeling.
      </p>
    </div>
    {/* Hosted by */}
