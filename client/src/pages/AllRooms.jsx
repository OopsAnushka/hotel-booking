...
const AllRooms = () => {
  ...
  const matchesRoomType = (room) => {
    return selectFilters.roomType.length === 0 || selectFilters.roomType.includes(room.roomType);
  }

  const matchesPricerange = (room) => {
    return selectFilters.priceRange.length === 0 || selectFilters.priceRange.some(range => {
      const [min, max] = range.split('to').map(Number);
      return room.pricePerNight >= min && room.pricePerNight <= max;
    });
  }

  // FIX: Updated logic to match 'sortOptions' strings exactly (Case Sensitive)
  const sortRooms = (a, b) => {
  ...
  // FIX: Added (rooms || []) safety check and corrected .filter()
  const filteredRooms = useMemo(() => {
    return (rooms || [])
      .filter(room => matchesRoomType(room) && matchesPricerange(room) && filterDestination(room))
      .sort(sortRooms);
  }, [rooms, selectFilters, selectSort, searchParams])
  ...
  return (
    <div className='flex flex-col-reverse lg:flex-row item-start justify-between pt-28 md:pt-35 md:px-16 lg:px-24 xl:px-32'>
      <div>
      ...
      {/* filters */}
      <div className='bg-white w-70 h-fit border rounded-lg shadow-sm border-gray-300 text-gray-600 max-lg:mb-8 min-lg:mt-16'>
        ...
        <div className={`flex items-center justify-between px-5 py-2.5  min-lg:border-b border-gray-300 ${openFilters && "border-b"}`}>
          ...
          <div className='text-xs cursor-pointer'>
            ...
            {/* FIX: Attached onClick handler here */}
            <span onClick={clearFilter} className='hidden lg:block'>CLEAR</span>
          </div>
        </div>
        <div className={`${openFilters ? 'h-auto' : "h-0 lg:h-auto"} overflow-hidden transition-all duration-700`}>
          <div className='px-5 pt-5'>
            <p className='font-medium text-gray-800 pb-2'>Poplar filters</p>
            {roomTypes.map((room, index) => (
              <CheckBox key={index} label={room} selected={selectFilters.roomType.includes(room)} onChange={(checked) => handleFilterChange(checked, room, 'roomType')} />
            ))}
          </div>
          <div className='px-5 pt-5'>
            <p className='font-medium text-gray-800 pb-2'>Price Range</p>
            {priceRanges.map((range, index) => (
              <CheckBox key={index} label={`$${currency} ${range}`} selected={selectFilters.priceRange.includes(range)} onChange={(checked) => handleFilterChange(checked, range, 'priceRange')} />
            ))}
          </div>
